...
export async function generateMetadata(
  ...
  const response = await fetch(
    `https://panel.ejarecantiner.com/api/articles?filters[url][$eq]=${params.url}&populate=*`,
    {
      method: "Get",
      headers: {
        Authorization:
    ...
    <>
      ...
      <main className=" w-full max-w-[1200px] flex flex-col gap-6 items-center justify-center mx-auto pt-10">
        ...
        >
          <Image
            src={
              "https://panel.ejarecantiner.com" + data.cover.data.attributes.url
            }
            alt={data.Title}
            width={1200}
            height={0}
            ...
            <div className="flex flex-col gap-3 items-start w-full md:px-8 lg:px-14 text-secondary pb-10 px-10">
              {data.RelatedLinks.map(
                (related: { link: string; linkTitle: string }) => {
                  return (
                    <Link
                      href={`https://ejarecantiner.com//${related.link}`}
                      key={related.link}
                    >
                      {related.linkTitle}
                    </Link>
